...
app.get('/home',
    ...
    expressQAsync(async (req: Request, res: Response, next: NextFunction) => {
        ...
        const response = createResponse("OK", { co2sav, totdist, rats, petlsav, grnmls, costrcv, batchrgper, rngcvr, rngavail }, undefined)
        res.json(response)
    })
)
//start a new ride
...
app.put('/rating',
    ...
    expressQAsync(async (req: Request, res: Response, next: NextFunction) => {
        ...
        const newride = await rateYourRide(frameId as string, startTime as string, endTime as string, rating as any as number)
        const response = createResponse("OK", newride, undefined)
        res.json(response)
    })
)
