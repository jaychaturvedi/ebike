...
export default class Ride {
    ...
    static async findById(id: number) {
        const ride = await RideModel.findOne({
            where: { id } })
        if (!ride) throw new Error('Unable to find by id ' + id);
        return ride
    ...
    static async createNew(ride: TRide) { //change types of userId
        ...
        if (!newride) throw new Error("Unable to create new")
        return newride;
    }
    ...
    static async updateById(id: number, ride: TRide) {
        ...
        if (!isUpdated) throw new Error("Unable to update with id ")
        return result
    }
    ...
    static async deleteById(id: number) {
        const deleted = await RideModel.destroy({
            where: { id } });
        if (!deleted ) throw new Error("Unable to delete with id " + id);
        return deleted
