...
import NextButton from './onboarding/components/next-page-button'

type Props = {}
type State = {}

export const getHeader = (skipButton: boolean, backButton: boolean, skipFunc?: () => void, backFunc?: () => void) => {
    return (
        <View style={styles.skip}>
            {backButton ? (<TouchableOpacity style={{ flex: 1 }}
                onPress={backFunc}
            ><Icon type="FontAwesome" name="chevron-left" style={{
                fontSize: scale(14),
            }} />
            </TouchableOpacity>) : null}
            {skipButton ? (<TouchableOpacity style={{ flex: 1, justifyContent: 'flex-end', flexDirection: 'row' }}
                onPress={skipFunc}
            ><Text style={{
                fontSize: scale(14),
            }}>Skip</Text>
...
export const getText = (text: string) => {
    return (
        <View style={styles.helpText}>
            <Text style={{ fontSize: scale(24), fontWeight: 'bold', textAlign: 'center' }}>{text}</Text>
        </View>
    )
}
