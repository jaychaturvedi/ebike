...
class DoubleLineGraph extends PureComponent<
  DoubleLineGraphProps,
  DoubleLineGraphStates
> {
  ...
  static getDerivedStateFromProps(
    props: DoubleLineGraphProps,
    state: DoubleLineGraphStates
  ) {
    let data = state.data;
  ...
  CustomTooltip = (obj: any) => {
    ...
    if (
      ...
      payload?.length === 0 ||
      !payload ||
      this.state.lineTooltipType === ""
    )
      return null;
    const formatType =
      this.props.xAxisLabel === "Days" ? "DD/MM/YYYY" : "DD/MM/YYYY hh:mm:ss a";
    const line = payload.filter((item: any) => {
      return item.dataKey === this.state.lineTooltipType;
    });
    const localAlertTime = formatDateTime(
      line[0]?.payload?.xAxisValue,
      formatType
    );
    ...
    return (
      <div className='custom-tooltip' style={style}>
        <p className='label'>
          {line[0]?.name} : {line[0]?.value}
        </p>
        <p className='label'>
          {this.props.xAxisLabel} : {`${localAlertTime}`}
        </p>
      </div>
    );
  };
    ...
    return (
      <div className='connectm-AlertDetailGraph'>
        <div className={"connectm-header"}>
          <Typography.Text className='graph-header-text' strong>
            {this.props.title}
          </Typography.Text>
        </div>
        {/* <LineGraph/> */}
        ...
        >
          <ResponsiveContainer width='95%' height='95%'>
            ...
            >
              ...
              {!this.props.alertCleared && this.state.L1Value ? (
                <ReferenceLine
              ...
              {!this.props.alertCleared && this.state.L2Value ? (
                <ReferenceLine
                  y={this.state.L2Value}
                  stroke={this.props.refColor}
                  strokeDasharray='3 3 5 2'
                  isFront={true}
                >
              ...
              >
                <Label
                  ...
                  style={{ padding: 5 }}
                  content={(props) => {
                    return this.DynamicLabel(props);
                  }}
                />
              </XAxis>
              <YAxis
              ...
              {!this.props.alertCleared ? (
                <Line
                  ...
                  activeDot={{
                    onMouseOver: (e: any) =>
                      this.setState({ lineTooltipType: "primaryValue" }),
                    onMouseLeave: (e: any) =>
                      this.setState({ lineTooltipType: "" }),
                  }}
                />
              ) : (
                ""
              ...
              {!this.props.alertCleared ? (
                <Line
                  ...
                  activeDot={{
                    onMouseOver: (e: any) =>
                      this.setState({ lineTooltipType: "secondaryValue" }),
                    onMouseLeave: (e: any) =>
                      this.setState({ lineTooltipType: "" }),
                  }}
                />
